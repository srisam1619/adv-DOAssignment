App.controller('doassignment_ctrl', ['$scope', '$rootScope', '$http', '$window', '$cookies','util_SERVICE','$location',
...
$scope.whatClassIsIt = function(status,driver)
{
	
		if(driver=="")
			return "orange";
		if(driver!="" && (status!="Cancelled" || status!="Completed"))
			return "Green";
		if(driver!="" && (status=="Cancelled" || status=="Completed"))
			return "blue";
...
$scope.searchmaster = function()
{
	...
	{
		US.GetPostalZoneandAssignedDO($scope.Fdate,$scope.Tdate,$scope.user[0].CompanyCode,$scope.TimofDelivery).then(function (response) 
		{
			$scope.masterDrivers = response.data;
			$scope.total = 0;
			if($scope.masterDrivers.length!=0)
			{
				for(var i=0;i<=$scope.masterDrivers.length;i++)
				{
					$scope.total = $scope.total+parseInt($scope.masterDrivers[i].AssignedDO);
				}
			}
			
		});
		
		US.Dosearch($scope.Fdate,$scope.Tdate,$scope.user[0].CompanyCode,$scope.statusss,$scope.TimofDelivery,$scope.driver1).then(function (response) 
		{
			for (var i = 0; i < response.data.length; i++)
			{

			  if(response.data[i].TimeofDelivery=='')
			  {
				  response.data[i].TimeofDelivery=$scope.ampm;
			  }
		  
	  		}
			$scope.DOSdata = response.data;
...
$scope.printdoUDF = function(sdoc)
{
	US.DOA_Print_UpdateUDF(sdoc,$scope.user[0].CompanyCode).then(function (response) 
				{
...
$scope.searchmaster();
	...
	US.DO_GetNewDrivers($scope.user[0].CompanyCode).then(function (response) 
	{
		$scope.NewDrivers = response.data;
		$scope.NewDrivers_array = $scope.getdervernameonly(response.data);
...
$scope.getdervernameonly = function(data)
	...
	$scope.updateDO = function()
	{
		...
		US.updateDOA_DO($scope.DOSdata,$scope.user[0].CompanyCode).then(function (response) 
				{
					$scope.updatebtn = false;
					$scope.result = response.data;
					if($scope.result[0].Result=="SUCCESS")
