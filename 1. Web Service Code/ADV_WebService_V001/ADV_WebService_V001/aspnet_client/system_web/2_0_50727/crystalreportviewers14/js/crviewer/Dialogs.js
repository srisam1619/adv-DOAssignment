...
bobj.crv.newPrintUI = function(kwArgs) {
    kwArgs = MochiKit.Base.update({id: bobj.uniqueId(), 
        width : (bobj.crv.config.isRTL ? 340 : 325),
        isExporting : false,
        isActxPrinting : false,
        height : 335},
    kwArgs);
...
bobj.crv.PrintUI._submitBtnCB = function() {
    ...
    if (this._rangeRadio.isChecked()) {
        ...
        if (!start || !end || (start < 0) || (start > end)) {
            MochiKit.Signal.signal(this, 'showError', L_bobj_crv_PrintPageRangeError);
            return;
        }
    }
...
bobj.crv.PrintUI._getObjectTag = function(postData) {
   ...
   // Add waiting UI while the control is loading
    ...
    oa.push('</td></tr>');
    oa.push('<tr><td align="' + (bobj.crv.config.isRTL ? 'right' : 'left') +'" style="padding-left:2px;padding-right:2px;padding-top:5px;">');
...
bobj.crv.PrintUI.getHTML = function(){
    ...
    if (!this.isActxPrinting) {
        html += "<table "+(bobj.crv.config.isRTL?"dir='rtl'":"")+" cellspacing=0 cellpadding=0 border=0>" +
                    "<tr>" +
                        "<td>" +
                            "<div class='dlgFrame'>" +
                    ...
                    "</tr>" +
                    "<tr>" +
                    	"<td align='"+ (bobj.crv.config.isRTL? "left":"right") +"' valign='top'>" +
                            "<table style='margin:6px 9px 6px 9px' cellspacing=0 cellpadding=0 border=0><tbody><tr>" +
                                "<td>" +                        
        ...
        html += "<div id='" + this.actxContainerId + "'></div>" +
                '<script for="' + this.actxId + '" EVENT="Finished(status, statusText)" language="javascript">' +
                    'DHTMLLIB.getWidgetFromID("' + this.id + '").show(false);' +
                '</script>' +
                '<script for="' + this.actxId + '" EVENT="PrintingProgress(pageNumber)" language="javascript">' +
                    'DHTMLLIB.getWidgetFromID("' + this.id + '")._processPrinting();' +
                '</script>' ;
    }
...
bobj.crv.newExportUI = function(kwArgs) {
    kwArgs = MochiKit.Base.update({ submitBtnLabel:L_bobj_crv_ExportBtnLbl,
                                    ...
                                    infoMsg:L_bobj_crv_PrintInfo1,
                                    isExporting:true, width : 310, height : 268}, kwArgs);
    ...
    if(o._comboBox) {
        ...
        o._comboBox.icon.h=18;
        o._comboBox.arrow.h=14;
    ...
    //Export Dropdown menu is centered
    if(_rtl)
    	o._comboBox.menu.isCentered = true;
...
bobj.crv.newErrorDialog = function(kwArgs) {
    ...
    o._separator = newSeparatorWidget(o.id+'_sep',null,4,2,4,2);
    return o;
...
bobj.crv.ErrorDialog.getHTML = function() {
    ...
    var errorText = convStr(this.text, false, true);
    
    var width = "320";	
    var detailWidth = "300px"; 
    var detailHeight = "100px"; var contentHTML = 
        TABLE({'class':"dlgBody", dir:(bobj.crv.config.isRTL?"RTL":"LTR"), width: width, cellpadding:"0", cellspacing:"5", border:"0"},
                TBODY(null,
                    TR(null, TD(null,
                        TABLE({'class':"dlgBody", cellpadding:"5", cellspacing:"0", border:"0"},                                       
                            TBODY(null,
                                TR(null, 
                                    TD({align:(bobj.crv.config.isRTL?"left":"right"), width:"32"}, 
                                        img(imgPath, 32, 32, null, 'id="dlg_img_' + this.id + '"', imgAlt)),
                                    TD(),
                                    TD({id:"dlg_txt_" + this.id, align:(bobj.crv.config.isRTL?"right":"left")},
                                        DIV({tabindex:"0"}, errorText))))))),
                    TR({id: this.id + '_detRow', style: {display: "none"}}, 
                        TD(null, DIV({'class': "infozone", dir:"ltr", style: {width: detailWidth, 'height': detailHeight, overflow: "auto"}},
                            PRE({id: this.id + '_detArea'}, this.detailText)))),                                    
                    TR(null, TD(null, this._separator.getHTML())),
                    TR(null, TD(null,
                        TABLE({cellpadding:"5", cellspacing:"0", border:"0", width:"100%"},
                            TBODY(null, 
                                TR(null,
                                    TD({align:(bobj.crv.config.isRTL?"right":"left")}, this._detailBtn.getHTML()),
                                    TD({align:(bobj.crv.config.isRTL?"left":"right")}, this.yes.getHTML())))))))); 
...
bobj.crv.ErrorDialog.setText = function (text, detailText) {
    if(bobj.crv.config.isRTL)
        text = text.replace(/]/g, "]&lrm;").replace(/\[/g, "&lrm;["); // Adding Left-To-Right markers to the brackets
...
bobj.crv.ErrorDialog.show = function(isShow, closeCB) {
    ...
    if (isShow) {
        this.lastFocusElement = bobj.getActiveElement();
