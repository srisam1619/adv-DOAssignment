...
bobj.crv.params.ParameterController.prototype = {
    ...
    setParameters: function(paramList, finishCB) {
        ...
        for (var i = 0; i < paramList.length; ++i) {
            ...
            if(isValueOptional)  {
                ...
                newParamWidgetArgs.isEmptyStringNoValue = (param.valueDataType != Type.STRING && param.valueDataType != Type.MEMBER); 
            }
            ...
            var updatePanel = function (panel) { 
                return function() { 
                    ...
                    if (MochiKit.Base.isIE ()){
                        ...
                        // In RTL mode, redraw causes text field to be drawn outside the prompt panel
    ...
    _getMinMaxText: function(type,minValue,maxValue) {
        ...
        if((type == Type.STRING) || (type == Type.MEMBER)) {
     ...
     * @param valueText   [String] The formatted value
     * @param descriptionText   [String] The formatted description
     *
     * @return the value to display, in the format of "valueText - description"
     */
    _formatDescriptionString: function(displayType, valueText, descriptionText) {
    ...
    _getDefaultValue: function(type, displayType, valueObj) {
        ...
        var descriptionText = bobj.crv.params.getDescription (valueObj);                        
        return this._formatDescriptionString(displayType, valueText, descriptionText);
    ...
    _getRangeDisplayText: function(param, value) {  
        ...
        var lowerBoundValueText = this._getValueText(param.valueDataType, value.beginValue);
        var lowerBoundValueDisplay = value.beginDisplay;
        
        var upperBoundValueText = this._getValueText(param.valueDataType, value.endValue);
        var upperBoundValueDisplay = value.endDisplay;


        displayText.lowerBound = {type : value.lowerBoundType, value : this._formatDescriptionString(param.defaultDisplayType, lowerBoundValueText, lowerBoundValueDisplay)};
        displayText.upperBound = {type : value.upperBoundType, value : this._formatDescriptionString(param.defaultDisplayType, upperBoundValueText, upperBoundValueDisplay)};
