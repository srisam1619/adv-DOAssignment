bobj.crv.GroupTreeListener = function(groupTree) {
    ...
    MochiKit.Signal.connect(groupTree.getCtrNode(), "onscroll", bobj.bindFunctionToObject(this.detectTreeChanges, this));
...
bobj.crv.GroupTreeListener.prototype = {
    ...
    getNumberOfNodesMissing : function() {
        return this._groupTree.getModalChildrenCount () - this._lastNodeRendererd - 1;
    },

    getTreeState : function() {
        var gt = this._groupTree.getCtrNode();
        return {
            height :gt.offsetHeight,
            scrollTop :gt.scrollTop,
            scrollHeight :gt.scrollHeight
        };
    },
    ...
    getNumberOfNodesToRender : function() {
        ...
        var lastNodeRendered = this._groupTree.getChildren(this._lastNodeRendererd);
        var treeNewState = this.getTreeState ();

        if (lastNodeRendered != null) {
            var offsetY = lastNodeRendered.layer.offsetTop;
            if (offsetY < treeNewState.scrollTop + (2 * treeNewState.height)) {
                var freeSpace = treeNewState.scrollTop + (2 * treeNewState.height) - offsetY;
    ...
    updateTreeChildren : function() {
        ...
        if (numNodesToRender > 0) {
            ...
            for ( var i = this._lastNodeRendererd + 1, lastNode = this._lastNodeRendererd + numNodesToRender; i <= lastNode; i++) {
                var modalChild = this._groupTree.getModalChild(i);
                if (modalChild != null) {
                    var treeNode = bobj.crv.createWidget (modalChild);
                    this._groupTree.delayedAddChildToRealGroupRoot (treeNode);
                    childrenHTML += treeNode.getHTML(0);
                    this._lastNodeRendererd  = i;
                }
            }
            
            this._groupTree.addRealGroupChildrenHTML(childrenHTML);
    ...
    isTreeStateChanged : function() {
        ...
        if (currentState.scrollTop != this._groupTreePrevState.scrollTop)
            return true;
        if (currentState.scrollHeight != this._groupTreePrevState.scrollHeight)
            return true;
    ...
    addFutureNodesPlaceHolder : function() {
        ...
        this._groupTree.appendToTreeCtr (this._futreNodesPlaceHolder);
    }
}
