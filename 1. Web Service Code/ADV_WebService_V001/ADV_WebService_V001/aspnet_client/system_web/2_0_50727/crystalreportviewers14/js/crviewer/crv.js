...
if (typeof(bobj.crv) == 'undefined') {
    ...
    (function () {
        ...
        bobj.crv.ActxPrintControl_CLSID = 'C05FE3D5-CD6A-4FBA-BBD5-54E3BDE35F07';
    bobj.crv.ActxPrintControl_Version = '14,1,3,1231';
        ...
        bobj.crv.config = {
            ...
            useAsync : true,
            indicatorOnly: false,
            isIncludeDefaultCSS : false,
            resources : { 
              'HTMLPromptingSDK' : {isLoaded: false, path : '../../promptengine-compressed.js', isEvalOnWindow : true}, 
              'ParameterControllerAndDeps' : {isLoaded: false, path: '../../parameterUIController-compressed.js', isEvalOnWindow : false}  
            },
         ...
         */
        bobj.crv.createWidget = function(json, skipChildren) { 
            ...
            if (!skipChildren && bobj.isArray(json.children)) {
         ...
         * @return Returns the widget or null if creation failed
         */
        bobj.crv.writeWidget = function(json, containerID) {
           
            var widget = bobj.crv.createWidget(json);
            if (widget) {
                if(containerID){
                    var element = DHTMLLIB.getLayer(containerID);
                    var html = widget.getHTML();
                    var ext = bobj.html.extractHtml(html);
                    var links = ext.links;
                    DHTMLLIB.append2(element, html);
        ...
        bobj.crv._includeAll = function() {
            ...
            if(bobj.crv.config.isIncludeDefaultCSS)
                bobj.crv._includeDefaultCSS();
            ...
            if (bobj.crv.config.useCompressedScripts) {
                if (bobj.crv.config.indicatorOnly) {
                    ...
                    for(var i = 0 ; i <  bobj.crv.config.apiResources.length; i++) {
                        var path = bobj.crv.config.apiResources[i];
                        bobj.crv._loadJavaScript(path);
                    }
                ...
                if (bobj.crv.config.indicatorOnly) {
                    ...
                    scripts = [
                        ...
                        'ToolPanel.js',
                        'ReportPage.js',
                        'ReportPageListener.js',
                        'CanvasListener.js',
                        'ScrollingReportPage.js',
                        'ReportView.js',
                        'ButtonList.js',
                        'ReportAlbum.js',
                        'ReportBreadcrumb.js',
                        ...
                        'SignalDisposer.js',
                        'ScrollingReportView.js',
                        'ReportViewListener.js',
                        'TakeAction.js',
                        'ColorPalette.js',
                        'ThemeManager.js',
                        'PublicArtifacts.js'
        ...
        bobj.crv._includeDefaultCSS = function () {
            document.write ('<li' + 'nk rel="stylesheet" type="text/css" href="' + bobj.crvUri('images/style.css') + '">');
        };
        ...
        bobj.crv.initLog = function(logLevel, handlerUri) {
            ...
            if (bobj.crv.config.logging.enabled) {            
                ...
                var oldLogFunc = MochiKit.Base.bind (bobj.crv.logger.log, bobj.crv.logger);
                bobj.crv.logger.log  = function(level, params) {
                    if(params.length > 0)
                    {
                        params[0] = bobj.crv.config.logging.id + ' ' + params[0]; 
                    }
                    oldLogFunc(level, params);
                };
