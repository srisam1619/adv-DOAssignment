...
bobj.crv.ServletAdapter.prototype = MochiKit.Base.merge(bobj.crv.IOAdapterBase, {
    ...
    request: function(pageState, viewerName, eventArgs, allowAsync, useIframe) {
        ...
        if (allowAsync && this._servletUrl) {
            deferred = bobj.crv.AsyncTransporter.getInstance().post(this._servletUrl, MochiKit.Base.queryString(this._form), null, null, {
    ...
    processError: function(response) { 
        ...
        else if (!(typeof(response.number) == 'undefined') && response.number == 500 && response.req && response.req.responseText) {
            return response.req.responseText.split('<body>').pop().split('</body>')[0];
        }
...
bobj.crv.FacesAdapter.prototype = MochiKit.Base.merge(bobj.crv.IOAdapterBase, {
    ...
    request: function(pageState, viewerName, eventArgs, allowAsync, useIframe) {
        ...
        if (allowAsync && this._servletUrl) {            
            deferred = bobj.crv.AsyncTransporter.getInstance().post(this._servletUrl, MochiKit.Base.queryString(form), null, null, {
