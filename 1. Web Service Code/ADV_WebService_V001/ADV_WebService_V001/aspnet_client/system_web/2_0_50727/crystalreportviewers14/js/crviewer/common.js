...
/**
 * @return [boolean] True if the hex color is a valid value
 */
bobj.isValidHex = function(color) {
...
bobj.isParentOf = function(child, parent) { 
    try {
        if (!parent || !child)
            return false;
        else if (child.parentNode == parent)
            return true;
        else if (child.parentNode == child)
            return false;
        else
            return arguments.callee(child.parentNode, parent);
    }
    catch(e) {
        return false;
    }
...
bobj.getEventTarget = function(e) {
...
bobj.loadJSResourceAndExecCallBack = function(resource, callback)
{
    ...
    if (!resource.isLoaded) {
        bobj.crv.AsyncTransporter.getInstance().get(bobj.crvUri(resource.path), null, /*onsuccess*/function(response) {
            resource.isLoaded = true;
            if(resource.isEvalOnWindow)
                bobj.evalInWindow (response.responseText);
            else
                bobj.evalScript(response.responseText);
            callback.apply();
        }, /*on fail*/null , {
            'Accept' : 'application/x-javascript, application/javascript, text/javascript'
        });
    }
    else {
...
bobj.includeCSSLinkAndExecuteCallback = function(href, callback) {
    ...
    bobj.crv.AsyncTransporter.getInstance().get(href, null, function(response) {
        bobj.addStyleSheet(response.responseText, cssLinkId);
        callback.apply();
    }, function(response) {
        callback.apply();
    }, {
...
bobj.getExecuteDOMCallbackHTML = function(widgetID, fnName) {
    return "return bobj.executeDOMCallback(" + widgetID + ", '" + fnName + "', event)";
};

bobj.getExecuteDOMCallback = function(widgetID, fnName) {
    return function(ev) {
        return bobj.executeDOMCallback(widgetID, fnName, ev );
    }
};

/*
 * Executes function [name = fnName] on widget [widx = widgetID]
 */
bobj.executeDOMCallback = function(widgetID, fnName, ev) {
    if (_widgets[widgetID] != null) {
        var widget = _widgets[widgetID];
        if (widget[fnName] != null) {
            var cb = widget[fnName];
                return cb.apply(widget, [ ev ]);
        }
    }
}
...
/*
 * Adds an event listener to DOM element
 * @param domE DOM element
 * @param eventName event name onmousemove, onclick, etc
 * @param listener event listener
 */
bobj.connectDOMEvent = function (domE, eventName, listener) {
    if(domE.addEventListener) {
        domE.addEventListener(eventName.substr(2), listener, false);
    }
    else if(domE.attachEvent) {
        domE.attachEvent(eventName, listener);
    }
};

/*
* Removes an event listener from DOM element
* @param domE DOM element
* @param eventName event name onmousemove, onclick, etc
* @param listener event listener
*/
bobj.disconnectDOMEvent = function (domE, eventName, listener) {
    if(domE.removeEventListener) {
        domE.removeEventListener(eventName.substr(2), listener, false);
    }
    else if(domE.detachEvent) {
        domE.detachEvent(eventName, listener);
    }
};

bobj.connectMouseWheelListener = function(elem, callback) {
    if (elem.addEventListener) {
        elem.addEventListener('DOMMouseScroll', callback, false); // FF
        elem.addEventListener('mousewheel', callback, false); // Opera, Safari
    } else
        elem.attachEvent('onmousewheel', callback); // IE
};

bobj.disconnectMouseWheelListener = function(elem, callback) {
    if (elem.removeEventListener) {
        elem.removeEventListener('DOMMouseScroll', callback, false); // FF
        elem.removeEventListener('mousewheel', callback, false); // Opera, Safari
    } else
        elem.detachEvent('onmousewheel', callback); // IE
};


bobj.logToConsole = function(msg) {
    if (bobj.crv.config.isDebug && window.console && window.console.log) {
        window.console.log(msg);
    }
};

bobj.getActiveElement = function () {
    try {
        // When viewer is inside a frame, IE throws "Unspecified" error.
