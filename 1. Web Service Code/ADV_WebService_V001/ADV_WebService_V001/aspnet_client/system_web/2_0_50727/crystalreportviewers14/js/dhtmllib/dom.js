...
DHTMLLIB = new function () {
    this.add = function(fn/*,fn2,fn3,etc*/) {
        var args = arguments;
        for(var i = 0; i < args.length; i++) {
            var fn = args[i];
            var name = fn.toString().split(/[\s\(]/g)[1];
            this[name] = fn;
        }
    };
};

var _dhtmlLib = "DHTMLLIB";

var _userAgent=navigator.userAgent?navigator.userAgent.toLowerCase():null
var _ie=(document.all!=null)?true:false
var _dom=(document.getElementById!=null)?true:false
var _isQuirksMode = (document.compatMode != 'CSS1Compat');
var _dtd4=!_ie||(document.compatMode!='BackCompat') // Browser is compatible HTML 4.0 Strict
var _moz=_dom&&!_ie
var _show='visible'
var _hide='hidden'
var _hand=_ie?"hand":"pointer"
var _appVer=navigator.appVersion.toLowerCase();
var _webKit=(_userAgent.indexOf("safari")>=0)||(_userAgent.indexOf("applewebkit")>=0)
var _firefox = (_userAgent.indexOf("firefox")>=0)
var _mac=(_appVer.indexOf('macintosh')>=0)||(_appVer.indexOf('macos')>=0);
var _opera = (_userAgent.indexOf('opera') != -1);     // Browser is opera
var _userAgent=navigator.userAgent?navigator.userAgent.toLowerCase():null
var _saf=_moz&&(_userAgent.indexOf("safari")>=0)
var _ctrl=0
var _shift=1
var _alt=2
//_def_shortcut=_mac?"&#8984;":
...
var _small=(screen.height<=600)
var _curDoc=document
var _curWin=self
var _tooltipWin=self
var _tooltipDx=0
var _tooltipDy=0
var _codeWinName="_CW"
var _leftBtn=(_ie||_saf)?1:0
var _preloadArr=new Array();
var _widgets=new Array();
var _resizeW=_ie6Up?"col-resize":"E-resize"
var _resizeH=_ie6Up?"row-resize":"S-resize"
var _ddData=new Array
var _dontNeedEncoding=null;
var _thex=null;
var _allGrabbers = new Array();
var _defaultButtonWidth=60;
var BGIFramePool = null;
var id = 0;
var _rtl = false;
...
function initDom(skin,style,lang,rtl,curWin,codeUniqueName)
...
// codeUniqueName [String - Optional] must be set if curWin specfied - enter a unique keyword
{
	...
	_dhtmlLib = _codeWinName + ".DHTMLLIB";
...
// y [int optional] new ordinate
{
	var c=this.css;	
	if (x!=null){
	    if (_rtl && this.respectPageDirection)
	        if (_moz) c.right=""+x+"px";else c.pixelRight=x;
	    else
	        if (_moz) c.left=""+x+"px";else c.pixelLeft=x;
	}
	if (y!=null){
		if (_moz) c.top=""+y+"px";else c.pixelTop=y			
	}
}
...
// PRIVATE
{
	...
	o.isFromButton=true
	lyr.onmouseup=eval('_curWin.'+_dhtmlLib+'.GrabberWidget_buttonup')
}
...
// PRIVATE
{
	...
	o.mod.onmousemove=eval('_curWin.'+_dhtmlLib+'.GrabberWidget_move')
	o.mod.onmouseup=eval('_curWin.'+_dhtmlLib+'.GrabberWidget_up')

	o.grabStartPosx=parseInt(_rtl?lyr.style.right:lyr.style.left)
	...
	var x=o.grabStartPosx
	var y=o.grabStartPosy
	ifr.move(x, y)
...
// PRIVATE
{
	...
	{	
		if (o.allowGrab)
		{
			var x=o.isHori?Math.max(0,_rtl?o.grabStartPosx-(eventGetX(e)-o.grabStartx):o.grabStartPosx-o.grabStartx+eventGetX(e)):null
...
function GrabberWidget_up(e)
{
	...
	{
		if (o.resizeCB)
			o.resizeCB(parseInt(_rtl?lyr.style.right:lyr.style.left),parseInt(lyr.style.top))
	}
}
...
var ButtonWidget_inst=new Array
var ButtonWidget_currInst=0
...
// returns [String] widget HTML
{
	with (this)
	{
		var clk=_dhtmlLib+'.ButtonWidget_clickCB('+this.instIndex+');return false;"'
		...
		clcbs+= 'onkeydown=" return '+_dhtmlLib+'.ButtonWidget_keydownCB(event,'+this.instIndex+');" '

		var addPar=' style="'+extraStyle+'cursor:'+_hand+';margin-left:'+margin+'px; margin-right:'+margin+'px; "'+clcbs+' ',tip=attr('title', tooltip),idText="theBttn"+id, idIcon="theBttnIcon"+id;
		
		var lnkB='<a '+attr('id',idText)+' '+tip+' '+attr('tabindex',tabIndex)+' href="javascript:void(0)" class="wizbutton" role="button">'
		var l=(label!=null)
		var im=(this.url?('<td align="'+(l?(this.imgRight?'right':'left'):'center')+'" width="'+(!l&&(width!=null)?width+6:w+6)+'">'+(l?'':lnkB)+simpleImgOffset(url,w,h,this.isGray?disDs:dx,this.isGray?disDy:dy,idIcon,null,(l?'':tooltip),'cursor:'+_hand)+(l?'':'</a>')+'</td>'):'')
		...
		return '<table class="wizbuttonBorder" onmouseover="return true" '+attr('id',id)+' '+addPar+' border="0" cellspacing="1" cellpadding="0"><tr valign="middle">'+
			(this.imgRight?'':im)+
			(l?('<td height="19" '+attr("width",width)+ attr('id',"theBttnCenterImg"+id)+' align="center" class="wizbuttonBG '+(this.isGray?'wizbuttongray':'wizbutton')+'" style="padding-left:8px;padding-right:8px;">'+
			'<nobr>'+lnkB+label+'</a></nobr></td>'):'')+
			(this.imgRight?im:'')+
			'</tr></table>';
	}
}
...
// PRIVATE internal click event handler
{
	...
	if (btn && !btn.isGray) {
		if(btn.isDelayCallback)
			setTimeout(_dhtmlLib + ".ButtonWidget_delayClickCB("+index+")",1);
		else
...
function ComboWidget_beginHTML()
{
	...
	return '<select '+(o.multi?'multiple':'')+' '+(o.noMargin?'style="'+sty("width",o.width)+_extrCmbS+'"':'style="'+sty("width",o.width)+'margin-left:10px;'+_extrCmbS+'"')+' class="listinputs" '+attr('onchange',_dhtmlLib+'.ComboWidget_changeCB(event,this)')+attr('onclick',_dhtmlLib+'.ComboWidget_clickCB(event,this)')+attr('ondblclick',_dhtmlLib+'.ComboWidget_dblClickCB(event,this)')+attr('onkeyup',_dhtmlLib+'.ComboWidget_keyUpCB(event,this)')+attr('onkeydown',_dhtmlLib+'.ComboWidget_keyDownCB(event,this)')+attr('id',o.id)+attr('name',o.id)+attr('title',o.tooltip)+'size="'+o.size+'">'
}
...
function InfoWidget_getHTML()
{
	...
	return '<div class="dialogzone" align="left" style="overflow:hidden;'+sty("width",o.width)+sty("height",""+o.height+"px")+'" id="'+o.id+'">'+
		'<nobr>'+img(_skin+'../help.gif',16,16,'top',null,L_DHTMLLIB_helpLab)+
...
function CheckWidget_getHTML()
{
	...
	'<tr valign="middle">'+
		'<td style="height:20px;width:21px"><input style="margin:'+(_moz?3:0)+'px" onclick="'+_dhtmlLib+'.CheckWidget_changeCB(event,this)" ' +
...
function newTextFieldWidget(id,changeCB,maxChar,keyUpCB,enterCB,noMargin,tooltip,width,focusCB,blurCB,clickCB)
{
	var o=newWidget(id)
	o.tooltip=tooltip
	o.changeCB=changeCB
...
function TextFieldWidget_init()
{
	var o=this
	o.wInit()	
	o.inputNode=getLayer(this.id+'_input');
	if (o.inputNode)
	    o.inputNode.value=""+ (o.oldValue != "")?o.oldValue : "";
...
{
	var o=this	
	return '<div id="'+this.id+'" style="position:relative;background-color:#ffffff;'+sty("width",this.width)+';"><div class="textinputsBG" style="width:100%"></div><div class="textinputsBorder">'
	    +'<input id="'+this.id+'_input"'+(o.disabled?' disabled':'')+' oncontextmenu="event.cancelBubble=true;return true" style="width:100%;'+(_ie?'height:16px;':'height:18px;')+'margin-left:'+(this.noMargin?0:10)+'px" onfocus="'+_dhtmlLib+'.TextFieldWidget_focus(this)" onblur="'+_dhtmlLib+'.TextFieldWidget_blur(this)" onchange="'+_dhtmlLib+'.TextFieldWidget_changeCB(event,this)" onkeydown=" return '+_dhtmlLib+'.TextFieldWidget_keyDownCB(event,this);" onkeyup=" return '+_dhtmlLib+'.TextFieldWidget_keyUpCB(event,this);" onkeypress=" return '+_dhtmlLib+'.TextFieldWidget_keyPressCB(event,this);" type="text" '+attr('maxLength',this.maxChar)+' ondragstart="event.cancelBubble=true;return true" onselectstart="event.cancelBubble=true;return true" class="crtextinputs" name="'+this.id+'"'+attr('title',this.tooltip)+' value="">'
	    +'</div></div>'
}
...
function TextFieldWidget_getValue()
{
	...
	{
		return o.inputNode ? o.inputNode.value : o.oldValue
	}	
}
...
{
	var o=this
	if (o.isHelpTxt && o.inputNode) o.inputNode.value= ""
	o.isHelpTxt = false
	
	if (o.inputNode)
	    o.inputNode.style.color="black"	
}
...
function TextFieldWidget_click(l)
//PRIVATE
{
	var o=getWidget(l)
	if (o.clickCB)
...
function TextFieldWidget_setHelpTxt(h)
{
	var o=this
	o.helpTxt=h		
	if (o.inputNode && (o.inputNode.value == "")) 
	{
		o.isHelpTxt=true
		o.inputNode.value=h
		o.inputNode.style.color="#808080"		
	}
}
...
{
	s=""+s
	var ret=s.replace(/&(?!rlm;|lrm;)/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")
...
function hasNoRTLCharacters (s)
{
	return (-1 == s.search (/[\u0590-\u074F\u0780-\u07BF\u08A0-\u08FA]/));
}
...
{	
	if (_ie)
	{
		if (dx==null) dx=0
		if (dy==null) dy=0
		return '<div '+(att?att:'')+' '+attr("id",id)+' '+(_rtl?'dir="ltr"':'')+' style="position:relative;padding:0px;width:'+w+'px;height:'+h+'px;overflow:hidden;'+(st?st:'')+'">'+img(url,null,null,(align?align:'top'),'style="margin:0px;position:relative;top:'+(-dy)+'px;left:'+(-dx)+'px" tabIndex="-1"',alt)+'</div>'		
	}
	else
...
function eventPreventDefault(e,win)
//Cancels event default behaviour
//e [event]
{
	win=win?win:_curWin;
	var ev =_ie? win.event : e;
...
{	
	...
	if (BGIFramePool) 
	{
		BGIFrames = BGIFramePool.split(",")
		var BGIFCount = BGIFrames.length
...
function holdBGIFrame(layerId) {
	...
	var id = parseInt(layerId.split('_')[1])
	var BGIFrames = BGIFramePool.split(",")
	BGIFrames[id]=1
	BGIFramePool = BGIFrames.join(",")
}
...
var _staticUnicBlockWhileWaitWidgetID = "staticUnicBlockWhileWaitWidgetID"
...
// Returns [String] the HTML
{
	return '<div id="'+ this.id+'" onselectstart="return false" ondragstart="return false" onmousedown="'+_dhtmlLib+'.eventCancelBubble(event)" border="0" hspace="0" vspace="0"  style="background-image:url('+_skin+'../transp.gif);z-index:5000;cursor:wait;position:absolute;top:0px;left:0px;width:100%;height:100%"></div>'
}
...
*/
DHTMLLIB.add(eventCancelBubble, ButtonWidget_delayClickCB, ButtonWidget_clickCB, ButtonWidget_keydownCB, ComboWidget_changeCB,
 ComboWidget_clickCB, ComboWidget_dblClickCB, ComboWidget_keyUpCB, ComboWidget_keyDownCB, TextFieldWidget_focus,
 TextFieldWidget_blur, TextFieldWidget_click, TextFieldWidget_changeCB, TextFieldWidget_keyDownCB, TextFieldWidget_keyUpCB,
 TextFieldWidget_keyPressCB, GrabberWidget_down, GrabberWidget_buttonup, GrabberWidget_move,
 GrabberWidget_up, CheckWidget_changeCB, append2, getLayer, getWidgetFromID, targetApp, hasNoRTLCharacters
);
// Exposing the _widgets array to WebElements as it is...
DHTMLLIB._widgets=_widgets;
