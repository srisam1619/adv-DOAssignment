...
var _allBOIcons=new Array
var _allBOIconsMenus=new Array
//_lastUsedColors=new Array
//_maxUsedColors=8

var _menuType_simple=0
var _menuType_color	=1
var _menuType_border=2
...
function newIconWidget(id,src,clickCB,text,alt,w,h,dx,dy,disDx,disDy,isTabEnabled,ariaHasPopUp, isHandleUpDownKey)
// CONTRUCTOR
...
// isTabEnabled : [boolean - optional] 	boolean for allowing tab control
{
	...
	o.isHandleUpDownKey = isHandleUpDownKey;
...
// disDy    : [int - optional]      vertical offset for disabled state
{
	...
	var tooltip = L_DHTMLLIB_openMenu.replace("{0}", (text?text:(alt?alt:"")));
	o.icon=newIconWidget("iconMenu_icon_"+id,src,IconMenuWidget_iconClickCB,text,alt,w,h,dx,dy,disDx,disDy,false, false, true/*isHandleUpDownKey*/)
	o.arrow=newIconWidget("iconMenu_arrow_"+id,_skin+"menus.gif",IconMenuWidget_arrowClickCB,null,tooltip,8,16,0,81,0,97,true, true)
	switch (menuType)
	{
...
function SingleIconMenuWidget_getHTML()
{
    ...
    return '<table id="'+o.id+'" cellspacing="0" cellpadding="0" border="0" style="'+d+'" role="button" class="'+o.nocheckClass+'"><tr>'+
           '<td>'+o.icon.getHTML()+'</td>'+
           '<td style="padding-left:'+o.spc+'px" width="'+(13+o.spc)+'">'+o.arrow.getHTML()+'</td>'+
           '</tr></table>';
}
...
function IconColorMenuWidget_changeTooltip(s)
{
	...
	if (o.icon && o.arrow) {
		...
		var tooltip = L_DHTMLLIB_openMenu.replace("{0}", s);
...
*/
// ================================================================================
function newPaletteVerticalSepWidget(id, height)
// CONSTRUCTOR
// id : [String] the icon id for DHTML processing
{
	var o=newSeparatorWidget(id,height,0,2,0,2)
	o.height = height;
	o.isSeparator=true
...
{
	var o=this
	var cm=o.contextMenu?('oncontextmenu="'+_dhtmlLib+'.PaletteContainerWidget_contextMenu(this,event);return false"'):''
...
// PRIVATE
{
	if (_ie)
		e=_curWin.event
	var delayedMenu=getWidget(o).contextMenu;
	
	setTimeout(function () {
	    delayedMenu.par = null;
	    delayedMenu.show(true, absxpos(e),  absypos(e));
	}, 1);
}
...
// return [String] the HTML
{
	...
	var alignLeft=_rtl ? 'right' : 'left'
	var alignRight=_rtl ? 'left' : 'right'
	...
	fields[j++]='<td width="100%" align="' + alignLeft + '" style="padding-left:'+o.vertPadding+'px;padding-right:4px">'
	fields[j++]='<table id="'+o.id+'_left" cellspacing="0" cellpadding="0"'+(o.isLeftTableFixed?'style="table-layout:fixed;width:100%"':"")+'>'
	fields[j++]='<tbody><tr valign="middle">'
	...
	for (var i=0;i<len;i++)
	{
		...
		{
			fields[j++]='</tr></tbody></table></td><td align="' + alignRight + '" style="padding-right:'+o.vertPadding+'px"><table cellspacing="0" cellpadding="0"><tbody><tr valign="middle">'
			haveRightZone=true
		}
...
function PaletteWidget_delayedAddChild(item, index)
//add an item into a palette. this element is a standard widget, with a valid
//getHTML function. For layout reasons, use only widget that are nor too high
//palette [PaletteWidget}
{
	...
	var tr = this.leftLayer.firstChild.firstChild;
...
{
	this.oldInit()
	this.leftLayer=getLayer(this.id+'_left');
...
{
	var o=this,imgCode=o.src?'<div style="position:relative;overflow:hidden;height:'+(o.h+o.border)+'px;width:'+(o.w+o.border)+'px;cursor:'+(o.clickCB ? (!o.acceptClick() ? 'default' : _hand):'default')+'">'+simpleImgOffset(o.src,o.w,o.h,o.dis?o.disDx:o.dx,o.dis?o.disDy:o.dy,'IconImg_'+o.id,null,o.alt,'margin:2px; background-repeat:no-repeat;')+o.extraHTML+'</div>':'<div class="iconText" style="width:1px;height:'+(o.h+o.border)+'px"></div>'
	...
	return '<table dir='+(_rtl?"RTL":"LTR")+' style="'+d+(o.width!=null?"width:"+o.width+"px;":"")+
	'margin:'+o.margin+'px" id="'+o.id+'"  class="' + o.nocheckClass + '" cellspacing="0" cellpadding="0"'+
	'border="0" role="button" '+(o.ariaHasPopUp?'aria-haspopup="true"':'')+'><tr valign="middle">'+
	'<td><div style="position: relative"><div class="button_image" style="width: '+buttonW+'px;height: '+buttonH+'px;"></div><div style="position:relative">'+
	((o.clickCB&&_ie)?lnk(imgCode,null,null,null, ' tabIndex="-1"' ):imgCode)+'</div></div></td>'+
	(o.text?'<td align="'+o.txtAlign+'" style="padding-left:'+(o.txtNoPadding?0:(o.src?4:2))+'px;padding-right:'+
	(o.txtNoPadding?0:2)+'px"><div id="IconImg_Txt_'+o.id+'" class="iconText'+(o.dis?"Dis":"")+'" '+txtAtt+'>'+convStr(o.text)+'</div></td>':'')+
	'</tr></table>'
}
...
{	
	...
	if(key == KEY_ENTER || key == KEY_SPACE || (o.isHandleUpDownKey && (key == KEY_UP || key == KEY_DOWN))) //enter or spacebar
	{
		if ((o.layer)&&(o.acceptClick()))
                {
		    o.internalUpCB()
		    o.layer.className=o.isHover?o.currentHoverClass:o.currentClass
		    o.setCrs();
		    setTimeout(function() {delayedClickCB(o.index)},1);        
		}
...
// dis : [String] if true, disables the widget
{
	...
	if (o.dis!=dis)
	{
		...
		if (o.layer)
		{
			...
			o.layer.setAttribute("aria-disabled", dis);
...
function CustomCombo_init()
{
	...
	var arrowToolTip = L_DHTMLLIB_openMenu.replace("{0}", (o.icon.alt?o.icon.alt:""));
...
function newComboTextFieldWidget(id,changeCB,maxChar,keyUpCB,enterCB,noMargin,tooltip,width,focusCB,blurCB)
{
	...
	o.focus = ComboTextFieldWidget_focus;
	o.inputNode=null
	
	return o
}

function ComboTextFieldWidget_focus()
{
    var o = this;
    safeSetFocus(o.inputNode);
}
...
function ComboTextFieldWidget_init()
{
	...
	o.oldInit()
	o.inputNode=getLayer(o.id+'_input')
	
	if (o.inputNode!=null)
	{
		o.setContentEditable(true)
		o.inputNode.onclick=ComboTextFieldWidget_onClick
	}
}
...
function newTextComboWidget(id,maxChar,tooltip,w,changeCB,checkCB,beforeShowCB,formName, role, ariaLabel)
// CONSTRUCTOR
// id			: [String]              the icon id for DHTML processing
...

{
	...
	o.role = role;
	o.ariaLabel = ariaLabel;
	// basic widgets
	o.text=newComboTextFieldWidget((formName?formName:"text_"+id),TextComboWidget_checkCB,maxChar,TextComboWidget_keyUpCB/*keyUpCB*/,TextComboWidget_enterCB,true,tooltip,w-13)
	o.isMenuDisabled = false;
	var tooltipText = L_DHTMLLIB_openMenu.replace("{0}", (tooltip?tooltip:""));
	o.arrow=newIconWidget("arrow_"+id,_skin+"menus.gif",TextComboWidget_arrowClickCB,null,tooltipText,8,16,0,81,0,97,true,true)
	...
	o.setMenuDisabled = TextComboWidget_setMenuDisabled;
...
function TextComboWidget_setMenuDisabled(disable) 
{
    var o = this;
    o.isMenuDisabled = disable;
    o.arrow.setDisplay(!disable);
}
// ================================================================================
...
{	
	...
	if(o.ariaLabel)
	    s += "<label style='display:none' id='" + o.id + "_label'>" + o.ariaLabel + "</label>";
	 
	s+='<table id="'+o.id+'" ' + (o.role ? ('role="' + o.role + '" ') : "") + (o.ariaLabel ? ('aria-labelledby="'+ o.id + '_label" ') : "") +' cellspacing="0" cellpadding="0" border="0" style="cursor:default;margin:'+o.margin+'px"><tbody><tr>'
...
// ================================================================================
function TextComboWidget_keyUpCB(e)
// PRIVATE

{
    var p=this.par;
    if(eventGetKey(e)==KEY_DOWN || eventGetKey(e) == KEY_UP) //enter or spacebar
    {
        p.clickCB();
    }
}
...
DHTMLLIB.add(PaletteContainerWidget_contextMenu, IconWidget_realOverCB, IconWidget_realOutCB);