    ...
    _convertFlexValue: function (fValue, type, dIndex) {
       var jsValue = {};
        if ((fValue.value !== undefined && fValue.value !== null)){
            ...
            var path = fValue.value.path;
            if (path !== undefined && path !== null)
            {
                jsValue.anncestors = [];
                for (var i = 0, len = path.length; i < len; i++) {
                    jsValue.anncestors.push(this._convertFlexValue(path[i], type, -1));
                }
            }
        	...
        	//Initializing jsValue.beginDisplay and jsValue.endDisplay
        	if (!fValue.beginDisplay){
        	    jsValue.beginDisplay = "";
        	}
        	if(!fValue.endDisplay){
        	    jsValue.endDisplay = "";
        	}
            // Range
            if (fValue.start){
                jsValue.lowerBoundType = fValue.start.inc == true ? 2 : 1;
                jsValue.beginValue = this._convertFlexValue(fValue.start, type, -1);
                if (dIndex > -1 && fValue.start.labels && fValue.start.labels.length > dIndex){
                    jsValue.beginDisplay = decodeURI(fValue.start.labels[dIndex]);
                }
            ...
            if (fValue.end){
                ...
                jsValue.endValue = this._convertFlexValue(fValue.end, type, -1);
                if (dIndex > -1 && fValue.end.labels && fValue.end.labels.length > dIndex){
                    jsValue.endDisplay = decodeURI(fValue.end.labels[dIndex]);
                }
