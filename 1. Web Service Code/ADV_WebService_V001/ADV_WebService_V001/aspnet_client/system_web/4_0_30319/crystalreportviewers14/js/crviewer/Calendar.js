...
bobj.crv.Calendar._createHeaderButtons = function() {
    ...
    this._prevMonthBtn = newIconWidget(this.id+"_pm",_skin+'../lov.gif',bind(this._onPrevMonthClick, this),"",L_DHTMLLIB_calendarPrevMonthLab,w,h,dx,dyDown, true);
    this._prevYearBtn = newIconWidget(this.id+"_py",_skin+'../lov.gif',bind(this._onPrevYearClick, this),"",L_DHTMLLIB_calendarPrevYearLab,w,h,dx,dyDown, true);
    this._nextMonthBtn = newIconWidget(this.id+"_nm",_skin+'../lov.gif',bind(this._onNextMonthClick, this),"",L_DHTMLLIB_calendarNextMonthLab,w,h,dx,dyUp, true);
    this._nextYearBtn = newIconWidget(this.id+"_ny",_skin+'../lov.gif',bind(this._onNextYearClick, this),"",L_DHTMLLIB_calendarNextYearLab,w,h,dx,dyUp, true);
...
bobj.crv.Calendar.getHTML = function() {
    ...
    var onkeydown = _dhtmlLib + ".MenuWidget_keyDown('" + this.id + "', event); return true";
    var onmousedown = _dhtmlLib + ".eventCancelBubble(event)";
    var onmouseup = _dhtmlLib + ".eventCancelBubble(event)";
    var onkeypress = _dhtmlLib + ".eventCancelBubble(event)";
    
    var dayHeaderAtt = {'class':"calendarTextPart"};
    
    // viewer workflow - use bobj.crv.config
    // CAF (CR2013) workflow - use bobj.prompt
    var isRTL = (bobj.crv.config && bobj.crv.config.isRTL) || (bobj.prompt && bobj.prompt.isRTL);
    var tableDir = isRTL ? "rtl" : "ltr";
    var buttonsAlign = isRTL ? "left" : "right";
   
    var html = TABLE({dir: tableDir, id: this.id, border:"0", cellpadding:"0", cellspacing:"0",
        ...
        TBODY(null, 
            ...
            TR(null, TD({align:"center"},
                TABLE({border:"0", cellspacing:"0", cellpadding:"0", style:{margin:"2px", 'margin-top': "6px"}},
                    ...
                    TR(null, TD({colspan:"7", align:buttonsAlign, style:{'padding-bottom':"3px", 'padding-top':"3px"}},
                        TABLE(null, TBODY(null, TR(null, 
                            TD(null, this._okBtn.getHTML()),
                            TD(null, this._cancelBtn.getHTML())))))))))));
                            
    return this._getLinkHTML('startLink_' + this.id, true) + html + this._getLinkHTML('endLink_' + this.id, false);                         
};
        ...
        TBODY(null,
            TR(null,
                ...
                TD({rowspan:"2", width:"100%", align:"center", 'class':"dialogzone"},
                    SPAN({id:this.id + "_month", tabIndex:"0"}, L_DHTMLLIB_month[this.date.getMonth()]),
                    "&nbsp;&nbsp;",
...
bobj.crv.Calendar._getSeparatorHTML = function() {
    var sep=newSeparatorWidget(bobj.uniqueId(),null,0,0,0,0);
    return sep.getHTML();
};

bobj.crv.Calendar._getLinkHTML = function(id, isFocusLastChild) {
    return bobj.html.A({
        id: id, 
        href: "javascript:void(0)", 
        onfocus: "MenuWidget_keepFocus('"+this.id+"'," + isFocusLastChild + ")", /*focus next month*/
        style:{
            visibility:"hidden", 
            position:"absolute",
            top : "-1000px",
...
bobj.crv.Calendar._update = function() {
    ...
    this._month.innerHTML = L_DHTMLLIB_month[this.date.getMonth()];
...
bobj.crv.Calendar.setShowTime = function(isShow) {
    ...
    if (this.layer) {
        ...
        this._timeRow.style.visibility = isShow ? 'visible' :'hidden';
    }
};
 ...
 */
bobj.crv.Calendar.focus = function(isFocusLastChild) {
    if (isFocusLastChild == undefined) {
        ...
        with(this.layer) {
            try {
                focusableElements = querySelectorAll("table[tabindex='0'], span[tabindex='0'], div[tabindex='0'], a");
            }
            catch(e) {
                focusableElements.push(getLayer(this.id+"_nm"));
            }
        }
    ...
    if(this.endLink) this.endLink.show(true);
    if(this.startLink) this.startLink.show(true);
};